...
{
    [Route("api/v{version:apiVersion}/[controller]")] // I didn't need to change the uri to "api/camps", the controller name works!
    [ApiVersion("2.0")]
    [ApiController]
    public class Camps2Controller : ControllerBase
    {
        ...
        public async Task<IActionResult> GetCamps(bool includeTalks = false)
        {
            try
            {
                ...
                return Ok(result); // if the type match, status Ok will be returned for us
            }
            catch (Exception)
            {
